...
app.get('/scrape', async (req, res) => {
    try {
        ...
        for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
            ...
            // Prepare update values (Ensuring column A remains the invoice link)
            const updateValues = [
                [
                    invoiceLink, // Ensure column A remains unchanged
            ...
            await sheets.spreadsheets.values.update({
                spreadsheetId: sheetId,
                range: `Sheet1!B${rowIndex + 1}`, // Start updating from column B to keep column A unchanged
                valueInputOption: 'RAW',
                resource: { values: updateValues }
            });
